...
- **현재 시스템(Baseline)과 최적화된 시스템(Optimized) 비교**
- 외부 API 지연/오류 상황에서 시스템 안정성 검증
- SLA 기준에 따른 응답 시간과 에러율 확인
### 사용자 정의 메트릭

| 메트릭 | 종류 | 목적 |
| --- | --- | --- |
| `req_duration_ms` | Trend | 요청 지연 시간 측정, p95 기준 성능 확인 |
| `errors_total` | Counter | 요청 실패 횟수 집계, 에러율 평가 |

### 테스트 시나리오

| 시나리오 | 목적 | VU 수 | 실행 시간 | 설명 |
| --- | --- | --- | --- | --- |
| **Warmup** | 시스템 데우기 | 0 → 50 | 1분 | 점진적으로 VU 증가, 서버 초기화 후 안정 상태 도달 |
| **Baseline** | 기존 환경 성능 측정 | 50 | 3분 | 현재(비최적화) 환경에서 동일 부하 실행 |
| **Optimized** | 최적화 환경 성능 측정 | 50 | 3분 | 최적화된 서버 환경에서 동일 부하 실행 |
| **Failure** | 외부 API 지연/오류 테스트 | 30 | 2분 | 외부 API 지연/오류 발생 시 서비스 안정성 확인 (`simulateKma=slow`) |
...
| 구분 | 항목 | 값 | 해석 |
| --- | --- | --- | --- |
| **핵심 지표** | 총 요청 수 | 103,865 | 테스트 동안 총 요청 수 |
|  | 성공 체크 | 103,864 / 103,865 (99.99%) | 거의 모든 체크 성공, 안정적 |
|  | 실패 체크 | 1 / 103,865 (0.001%) | 극히 소수 실패, 무시 가능 수준 |
|  | 평균 반복 시간 | 220.77ms | 요청 처리 속도 빠름 |
|  | 최대 반복 시간 | 30.2s | 일부 iteration 지연 발생, 외부 API 가능성 |
| **HTTP 요청 지표** | 평균 응답 시간 | 11.36ms | 매우 빠른 응답 |
|  | p90 | 18.27ms | 90% 요청이 18ms 이하로 응답 |
|  | p95 | 26.71ms | 95% 요청이 26ms 이하로 응답 |
|  | 최대 응답 시간 | 631.14ms | 일부 요청 지연 존재 |
|  | 실패율 | 0.00% (3/103,865) | 거의 모든 요청 성공 |
| **Thresholds** | errors_total | 0.001302/s < 0.02 | 기준 이하, 안정적 ✅ |
|  | req_duration_ms p95 | 26.71ms < 5000ms | 기준 이하, 빠른 응답 ✅ |
| **실행 정보** | VUs | 2 ~ 50 | 최대 50 VU, 평균 2 VU |
|  | Iterations | 103,864 | 반복 수, 초당 135 req/s |
|  | 단계별 진행 | warmup 100%, baseline 100%, optimized 100%, failure 100% | 모든 시나리오 정상 완료 |
|  | 전송 데이터 | 송신 21 MB, 수신 70 MB | 네트워크 부하 적절 |
|  |  |  |  |


### 결론

- 스트레스 안정적, 50 VU 부하에는 정상 동작
- 일부 iteration 최대 30s → 외부 API 지연 가능

### 성능 고도화 및 2차 테스트 진행 방향

1. **VU와 부하 확장**
    - 외부 API는 제한(100초당 100 요청)에 맞춰 pacing 적용
2. **Think time / Pacing 조절**
    - `sleep()` 사용으로 VU당 요청 속도 제한
    - 외부 API 요청과 내부 연산 분리 가능
3. **요청 병렬 처리 최적화**
    - 동일 요청 반복 시 내부 캐싱 활용
    - 서비스 레이어에서 캐시 적용 시, k6 테스트는 캐시 효과만 확인
4. **지연 대응 로직**
    - 외부 API 지연 대비: `http.batch()`, timeout 옵션, retry 제한
    - 실패 시 fail-fast, 로그 기록
5. **데이터 전송 최적화**
    - 불필요한 response body 제거
...
| 항목                               | 첫 번째 테스트 (500건, VU 50) | 두 번째 테스트 (103,865건, VU 2~50) | 비교 / 코멘트                  |
| -------------------------------- | ---------------------- | ---------------------------- | ------------------------- |
| **Errors (전체)**                  | 0 / 500 (0%)           | 1 / 103,865 (0.0013%)        | 두 번째 테스트도 거의 에러 없음, 안정적   |
| **Thresholds: errors_total**     | rate < 0.05 ✅          | rate < 0.02 ✅                | 두 테스트 모두 만족               |
| **Thresholds: req_duration p95** | 477.08ms ✅             | 26.71ms ✅                    | 두 번째 테스트가 훨씬 빠름           |
| **req_duration 평균**              | 69.94ms                | 11.35ms                      | 두 번째 테스트가 더 빠름            |
| **req_duration 최댓값**             | 490.94ms               | 631.14ms                     | 두 번째 테스트에서 일부 극단값 존재      |
| **req_duration p90**             | 131.79ms               | 18.27ms                      | 대부분 요청이 훨씬 빠름             |
| **Checks 성공률**                   | 100%                   | 99.99%                       | 매우 안정적, 한 건 실패 발생         |
| **HTTP 요청 횟수**                   | 500                    | 103,865                      | 두 번째 테스트는 훨씬 큰 부하         |
| **Iterations 평균 시간**             | 1.07s                  | 220.77ms                     | 두 번째 테스트 루프가 훨씬 짧음        |
| **VU**                           | 50 (고정)                | 2~50 (변동)                    | 첫 번째는 고정 50, 두 번째는 점진적 증감 |
| **데이터 전송량**                      | 수신 334 kB / 송신 95 kB   | 수신 70 MB / 송신 21 MB          | 두 번째 테스트는 큰 부하 처리         |
| **초당 처리량 (req/s)**               | 46.34                  | 135.20                       | 두 번째 테스트에서 처리량 크게 증가      |

<br>

### 결과
- **응답 속도**
    - 두 번째 테스트에서 평균 및 p95 응답 속도가 훨씬 빠름 → 아마 VU가 적거나 테스트 환경이 더 안정적이었기 때문.
    - 다만 최대값(max) 631ms가 관찰되어 일부 요청에서 지연 발생.
- **안정성**
    - 첫 번째 테스트: 에러 없음
    - 두 번째 테스트: 1건 에러 → 무시 가능 수준
- **부하 처리**
    - 두 번째 테스트는 요청 수가 훨씬 많고, VU 변화가 있었음에도 안정적 성능 유지

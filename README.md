...
| 항목                       | 첫 번째 테스트             | 두 번째 테스트              | 비교 / 향상율                         |
| ------------------------ | -------------------- | --------------------- | -------------------------------- |
| Errors (전체)              | 0 / 500 (0%)         | 1 / 103,865 (0.0013%) | 두 번째 테스트도 거의 에러 없음, 안정적          |
| Thresholds: errors_total | rate < 0.05 ✅        | rate < 0.02 ✅         | 두 테스트 모두 만족                      |
| req_duration p95         | 477.08ms             | 26.71ms               | 약 94.4% 감소, 지연(latency) 크게 개선    |
| req_duration 평균          | 69.94ms              | 11.35ms               | 약 83.8% 감소, 평균 응답 시간 최적화         |
| req_duration 최댓값         | 490.94ms             | 631.14ms              | 최대값 약 28.5% 증가, 일부 극단값 존재        |
| req_duration p90         | 131.79ms             | 18.27ms               | 약 86.1% 감소, 대부분 요청에서 지연 최소화      |
| Checks 성공률               | 100%                 | 99.99%                | 안정적, 실패율 0.001% 수준               |
| HTTP 요청 수                | 500                  | 103,865               | 요청 수 약 20,673% 증가, 대규모 부하 테스트 가능 |
| Iteration 평균 시간          | 1.07s                | 220.77ms              | 약 79.3% 감소, 루프 처리 시간 단축          |
| VUs                      | 50 (고정)              | 2~50 (변동)             | 첫 번째는 고정 50, 두 번째는 점진적 증감        |
| 데이터 전송량                  | 수신 334 kB / 송신 95 kB | 수신 70 MB / 송신 21 MB   | 전송량 약 20,967% 증가, 대용량 처리 확인      |
| 초당 처리량 (req/s)           | 46.34                | 135.20                | 약 191.7% 증가, 처리량(throughput) 향상  |

...
/**
 * WeatherMapper 외부 API(WeatherItem 리스트) 또는 DB 엔티티(Weather) 데이터를 애플리케이션 내부 DTO(UserWeatherResponse)나 엔티티로 변환하는 유틸 클래스
 */

public class LocalWeatherMapper {

    /**
     * WeatherItem 리스트 + Region 객체를 기반으로 UserWeatherResponse DTO 생성
     *
    ...
    public static UserWeatherResponse toUserWeatherResponse(List<LocalWeatherItem> items, Object region) {
        String province = "", city = "", district = "";

        // region 객체가 Region 엔티티인 경우 필드 추출(instanceof 패턴 매칭을 이용한 변수 바인딩 사용)
        ...
        for (LocalWeatherItem item : items) {
            switch (item.category()) {
                case "T1H":
                    temp = Double.valueOf(item.obsrValue());
                    break;
                case "REH":
                    hum = Double.valueOf(item.obsrValue());
                    break;
                case "RN1":
                    rain = Double.valueOf(item.obsrValue());
                    break;
                case "WSD":
                    wind = Double.valueOf(item.obsrValue());
                    break;
                case "PTY":
                    pty = mapPty(item.obsrValue());
                    break;
                case "SKY":
                    sky = mapSky(item.obsrValue());
                    break;
            }
        }
    ...
    /**
     * WeatherItem 리스트 + Region 객체를 기반으로 Weather 엔티티 생성 (DB 저장용)
     *
    ...
    public static LocalWeather toWeather(List<LocalWeatherItem> items, Region region) {
        Double temp = null, hum = null, wind = null, rain = null;
